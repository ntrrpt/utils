...
parser.add_option('-m', dest='months',   default='1-12',       help='month interval (2,4,10-12)')
parser.add_option('-d', dest='days',     default='1,15',       help='dayss interval (1,15)')
parser.add_option('-p', dest='proxy',    default='http://127.0.0.1:10809',           help='proxy for twikit / aria2')
...
def picsdump(tweets):
    ...
    for tweet in tweets:
        for _, media in enumerate(tweet.media):
            ...
            full_id = "%s_%s" % (media_id, all_ids.count(media_id)) # 1741609594680725580_1
            base_name = "%s %s" % (tweet.user.screen_name, full_id) # himelabo_chika 1741609594680725580_1

            match media.type:
                case "photo":
                    url = media_url.replace(f".{media_ext}", "?format=%s&name=orig" % media_ext)
...
for y in years:
    ...
    for m in months:
        ...
        for d in days:
            ...
            sd = loop.run_until_complete(searchdump(f'{options.search} lang:ja until:20{y}-{m}-{d}'))
